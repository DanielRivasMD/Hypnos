...
// metaEntry mirrors the JSON you wrote in mailCmd
...
var statusCmd = &cobra.Command{
	Use:   "status",
	Short: "List all downtime timers and their state",

	////////////////////////////////////////////////////////////////////////////////////////////////////

	Run: func(cmd *cobra.Command, args []string) {
		const op = "hypnos.status"
		...
		entries := make([]metaEntry, 0, len(fis))
		for _, fi := range fis {
		...
		for _, m := range entries {
			...
			age := now.Sub(m.InvokedAt).Truncate(time.Second)
			fmt.Fprintf(w, "%s\t%d\t%s\t%s\t%s\n",
				m.Name,
				m.PID,
				m.InvokedAt.Format("2006-01-02 15:04:05"),
