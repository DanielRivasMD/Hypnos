...
// mailCmd is the user-facing command. It either accepts all flags manually:
//
//	hypnos mail --duration 5s --log in-vivo --name in-vivo --script 'open -a Mail'
//
// or it loads defaults from a TOML:
//
//	hypnos mail --config mail
var mailCmd = &cobra.Command{
	Use:   "mail",
	Short: "Invoke a managed downtime timer",
	Long: `Invoke a short-lived worker that waits, then runs your script
...
// mailRunCmd is the hidden worker. It sleeps, execs your command, sends notification, then exits.
var mailRunCmd = &cobra.Command{
	Use:    "mail-run",
	Hidden: true,
	Run: func(cmd *cobra.Command, args []string) {
		const op = "hypnos.mail-run"
		...
		// pid file
		pidFile := filepath.Join(base, "daemons", runName+".pid")
		pid := os.Getpid()
		horus.CheckErr(
...
func init() {
	// user-facing flags
	mailCmd.Flags().StringVarP(&configName, "config", "c", "", "load workflow from ~/.hypnos/config/<name>.toml")
	mailCmd.Flags().StringVarP(&dreamName, "name", "n", "", "instance name (manual or default: <config>-<ts>)")
	mailCmd.Flags().StringVarP(&logName, "log", "l", "", "log file basename (no .log)")
	mailCmd.Flags().StringVarP(&scriptPath, "script", "s", "", "shell command to execute")
	mailCmd.Flags().DurationVarP(&duration, "duration", "t", time.Hour, "how long to wait")
	rootCmd.AddCommand(mailCmd)

	// hidden worker flags
	mailRunCmd.Flags().StringVar(&runName, "name", "", "instance name")
	mailRunCmd.Flags().StringVar(&runLog, "log", "", "log basename")
	mailRunCmd.Flags().StringVar(&runScript, "script", "", "shell command to execute")
	mailRunCmd.Flags().DurationVar(&runDuration, "duration", time.Hour, "how long to wait")
	rootCmd.AddCommand(mailRunCmd)
}

////////////////////////////////////////////////////////////////////////////////////////////////////

// spawnDream forks off a new "mail-run" worker process, piping its output into the log.
